...
def quantization_hardware(input_tensor,bit_num=3,upper_bound=1,noise_std=0.1,device='cpu'):
    #对输入的张量进行指定bit数目的量化

    interval=upper_bound/(2**bit_num-1)
    half_interval=interval/2
    clamp_tensor=torch.clamp(input_tensor, min=(-1)*upper_bound, max=upper_bound).to(device)
    int_interval=torch.round(clamp_tensor/interval).to(device)
    quantization_tensor=int_interval*interval
    output_tensor=quantization_tensor*(1+torch.normal(mean=0.0,std=noise_std, size=quantization_tensor.shape).to(device))
...
def real_Binarize(tensor,interval=0.1,slope=0.5):   #自己定义一个新的二值化的方程  根据单元结构进行拟合得出来的
   #tensor[tensor>interval]=(tensor[tensor>interval]-interval)*0.0000+1
   #tensor[tensor<-interval]=(tensor[tensor<-interval]+interval)*0.0000-1
   #tensor[(-interval<=tensor) & (tensor<=interval)]*=(1/interval)  
   #避免原地修改self.weight.org的值
    temp=torch.zeros_like(tensor)
    temp[tensor>interval]=(tensor[tensor>interval]-interval)*slope + 1.0
    temp[tensor<-interval]=(tensor[tensor<-interval]+interval)*slope-1.0
    temp[(-interval<=tensor) & (tensor<=interval)]=tensor[(-interval<=tensor) & (tensor<=interval)]*(1/interval)

    return temp

def device_Binarize(tensor,Ap = 59.01, Bp = 59.40, An = -63.56, Bn = -63.23):
    #根据器件特性输出二值化特点
    temp=torch.zeros_like(tensor)
    temp[tensor>=0.0]=2.0*( (Ap*tensor[tensor>=0.0]+1) / (Bp*tensor[tensor>=0.0]+2) - 0.5)
    temp[tensor<0.0] = -2.0*( (An*tensor[tensor<0.0]+1) / (Bn*tensor[tensor<0.0]+2) - 0.5)

    return temp

def device_Binarize_non_symmetry(tensor):
    #根据新的模型输出二值化特点
    temp=torch.zeros_like(tensor)
    temp=(1*tensor)/(1*tensor+100)

    return temp


def device_Binarize_sub_th(tensor):
    #器件部分区域处于亚阈值区，部分区间变化很小，几乎一直为零
    
    temp=torch.zeros_like(tensor)
    
    bound=0.2  #一直为零的区间边界范围 
...
class BinarizeLinear(torch.nn.Linear):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            #只在训练的时候对权重进行器件二值化，即self.weight.org还是采用模拟值
            #self.weight.data=Binarize(self.weight.org)
            self.weight.data = device_Binarize(tensor = self.weight.org )  #带入器件真实的二值函数  
            
            #以下是用于测试相关其他可能的单元结构模型的内容
            #self.weight.data=device_Binarize_non_symmetry(self.weight.org) #带入不对称的二值函数
            #self.weight.data=device_Binarize_sub_th(self.weight.org)
            
            
        #测试的时候进行器件噪声代入
        elif not self.training:
            pass
        #self.weight.data.add_(torch.normal(mean=0.0, std=0.03528, size=self.weight.data.shape).to(self.weight.data.device))
        #self.weight.data=real_Binarize(self.weight.org)  #代入器件真实的二值化函数
...
class BinarizeConv2d(torch.nn.Conv2d):
    ...
    def forward(self, input):
        if input.size(1) != 3:
            input.data = Binarize(input.data)
        if not hasattr(self.weight,'org'):  #在这里完成了原始的权重的赋值过程
            self.weight.org=self.weight.data.clone()
        self.weight.data=Binarize(self.weight.org)
...
class BNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in range(self.hidden_layers+1):
            ...
            #以下是指的Instance Normalization的内容
...
def train(model, train_loader, current_task_index, optimizer, device, args,
          prev_cons=None, prev_params=None, path_integ=None, criterion = torch.nn.CrossEntropyLoss(), quantization_use=True):
    
    model.train()
    if train_loader == cmnist_train_loader:
        for i,data in enumerate(train_loader):
            image, labels = data['image'], data['label']
            image=image[:,0,:,:].view(image.size()[0],-1)
            ...
            for p in list(model.parameters()):  # updating the org attribute
                if hasattr(p,'org'):
                    ...
                    if quantization_use:
                        ...
                        p.org.copy_(quantization_hardware(p.data,bit_num=6,noise_std=0.0,device=device))
    ...
    if quantization_use:
        for p in list(model.parameters()):  # updating the org attribute
            if hasattr(p, 'org'):
                p.org.copy_(quantization_hardware(input_tensor = p.data, bit_num=args.bit_num, upper_bound=args.upper_bound,noise_std=args.noise_std, device=device))  #对p.org进行量化处理



def test(model, args,test_loader, device, criterion = torch.nn.CrossEntropyLoss(reduction='sum'), verbose = False):
    ...
    #在进行测试的时候将其加入一些噪声，避免每次都要重新进行相加
    for p in model.parameters():
        if hasattr(p,'org'):
            #p_temp=p.org+torch.normal(mean=0.0,std=0.2, size=p.org.shape)  #加上相关读取噪声
   
            p.data=device_Binarize(tensor = p.org, Ap = args.Ap, Bp = args.Bp, An = args.An, Bn = args.Bn)  #进行器件化的二值化，这里也可以对器件的读取噪声进行处理
    
            #p.data=Binarize(p.org) #对二值化进行近似，只是在测试的时候进行
            #进行相关的其他模型函数的测试
            #p.data=device_Binarize_non_symmetry(p.org)  #带入不对称的二值化函数
            #p.data=device_Binarize_sub_th(p.org)
            
            #self.weight.data=real_Binarize(self.weight.org)
            #p.data.add_(torch.normal(mean=0.0, std=0.03528, size=p.data.shape).to(p.data.device))
    #cmnist不用修改

    if test_loader == cmnist_train_loader:
        with torch.no_grad():
            for i,data in enumerate(test_loader):
                ...
                output = model(image)
                test_loss += criterion(output, labels).item() # mean batch loss
    ...
    elif test_loader == cmnist_test_loader:
        with torch.no_grad():
            for i,data in enumerate(test_loader):
                image, labels = data['image'], data['label']
                image=image[:,0,:,:].view(image.size()[0],-1)
    ...
    #dataloader.datatset 是指的传进来的dataset
    if verbose :
